...
export default function AttendanceReport({ onExport }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`/dashboard/attendance/attendance-stats?period=${period}`);
    ...
    return (
        <div className={`${isDark ? 'bg-DarkBG3 text-TextLight' : 'bg-LightBG1 text-TextDark'} p-6 rounded-lg shadow-md`}>
            <AttendanceReportHeader onExport={onExport} />
